...
int start_silent_reset=0;
EXPORT_SYMBOL(start_silent_reset);
...
{
	if(start_silent_reset)
	{
		//printk("[%s]silent reset return \n",__func__);
		return -EFAULT;
	}
...
//		return IRQ_HANDLED;
	...
	if(mailbox == 0xabcdabcd)
	{
		cp_boot_ok=1;
		printk("CP BOOT OK \n");
		if(start_silent_reset==1)
		{
			start_silent_reset=0;
			printk("Silent Reset Done \n");
		}
	}
